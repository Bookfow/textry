...
import { ChevronLeft, ChevronRight, Minus, Plus, List, AlignLeft, AlignJustify, Settings2, Focus, Highlighter, MessageSquare, Trash2, X } from 'lucide-react'
...
interface TextBlock {
  type: BlockType
  content: string
  blockId?: string  // document_blocks의 block_id
}
...
function deserializeBlocks(text: string, pageNum?: number): TextBlock[] {
  if (!text) return []
  const parts = text.split('\n\n')
  let idx = 0
  return parts.map(part => {
    const blockId = pageNum ? `p${String(pageNum).padStart(3, '0')}_b${String(idx++).padStart(3, '0')}` : undefined
    if (part.startsWith('<h1>') && part.endsWith('</h1>')) return { type: 'heading1' as BlockType, content: part.slice(4, -5), blockId }
    if (part.startsWith('<h2>') && part.endsWith('</h2>')) return { type: 'heading2' as BlockType, content: part.slice(4, -5), blockId }
    if (part.startsWith('<h3>') && part.endsWith('</h3>')) return { type: 'heading3' as BlockType, content: part.slice(4, -5), blockId }
    if (part === '<hr>') return { type: 'separator' as BlockType, content: '', blockId }
    return { type: 'body' as BlockType, content: part, blockId }
...
export default function ReflowViewer({
    ...
    const loadTexts = async () => {
      ...
      if (documentId) {
        try {
          ...
          let texts = new Map<number, string>()
          let total = 0
          ...
          if (!blocksErr && blocks && blocks.length > 0) {
            // 블록 → 페이지별 텍스트로 재조립 (기존 형식 호환)
            const pageMap = new Map<number, string[]>()
            for (const b of blocks) {
              if (!pageMap.has(b.page_number)) pageMap.set(b.page_number, [])
              const arr = pageMap.get(b.page_number)!
              if (b.block_type === 'heading1') arr.push('<h1>' + b.content + '</h1>')
              else if (b.block_type === 'heading2') arr.push('<h2>' + b.content + '</h2>')
              else if (b.block_type === 'heading3') arr.push('<h3>' + b.content + '</h3>')
              else if (b.block_type === 'separator') arr.push('<hr>')
              else if (b.block_type === 'html') arr.push(b.content)
              else arr.push(b.content)
            }
            for (const [pn, parts] of pageMap) {
              texts.set(pn, parts.join('\n\n'))
            }
            total = texts.size
          } else {
            // 폴백: document_pages_text
            const { data, error } = await supabase
            ...
            if (!error && data && data.length > 0) {
              for (const row of data) texts.set(row.page_number, row.text_content)
              total = data.length
            }
          }
  ...
  const currentBlocks = deserializeBlocks(pageTexts.get(pageNumber) || '', pageNumber)
  ...
  const saveHighlight = async (color: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('highlights').insert({
        ...
        reader_id: user.id,
        block_id: pendingSelection.blockId,
  ...
  const renderHighlightedText = (text: string, blockId?: string) => {
    ...
    const blockHighlights = highlights.filter(h => h.block_id === blockId && h.page_number === pageNumber)
    if (blockHighlights.length === 0) return text

    // offset 기준 정렬
    const sorted = [...blockHighlights].sort((a, b) => a.start_offset - b.start_offset)
    const parts: React.ReactNode[] = []
    let lastEnd = 0

    for (const hl of sorted) {
      if (hl.start_offset > lastEnd) {
        parts.push(text.slice(lastEnd, hl.start_offset))
      }
      const hlStart = Math.max(hl.start_offset, lastEnd)
      const hlText = text.slice(hlStart, hl.end_offset)
      if (hlText) {
        parts.push(
          <mark key={hl.id} style={{ backgroundColor: HIGHLIGHT_COLORS[hl.color] || HIGHLIGHT_COLORS.yellow, color: 'inherit', borderRadius: '3px', padding: '1px 2px', cursor: 'pointer', boxDecorationBreak: 'clone' as any, WebkitBoxDecorationBreak: 'clone' as any }}
  ...
  const renderBlock = (block: TextBlock, index: number) => {
    ...
    switch (block.type) {
      case 'heading1':
        return (
          <h2 key={index} data-block-id={block.blockId} className={`font-bold mt-10 mb-4 ${focusCursor}`}
            ...
            {renderHighlightedText(block.content, block.blockId)}
          </h2>
        )
      case 'heading2':
        return (
          <h3 key={index} data-block-id={block.blockId} className={`font-bold mt-8 mb-3 ${focusCursor}`}
            onClick={(e) => handleBlockClick(index, e)}
      ...
      case 'heading3':
        return (
          <h4 key={index} data-block-id={block.blockId} className={`font-semibold mt-6 mb-2 ${focusCursor}`}
            onClick={(e) => handleBlockClick(index, e)}
      ...
      default:
        return (
          <p key={index} data-block-id={block.blockId} className={`mb-4 ${focusCursor}`}
            onClick={(e) => handleBlockClick(index, e)}
    ...
    <div className="h-full flex flex-col" style={{ backgroundColor: themeStyle.pageBg }}>
          ...
          <div className="relative w-72 max-w-[80vw] h-full flex flex-col shadow-2xl" style={{ backgroundColor: themeStyle.bg }}>
            ...
            <div className="flex-1 overflow-y-auto">
              {tocItems.length > 0 ? tocItems.map((item, i) => (
                ...
                <p className="px-4 py-8 text-center text-sm" style={{ color: themeStyle.muted }}>목차 정보가 없습니다</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: REFLOW_MAX_WIDTH, margin: '0 auto', padding: `2rem ${marginSize}px` }}>
          ...
          <div>
            {(() => {
              ...
              // HTML 콘텐츠면 dangerouslySetInnerHTML로 직접 렌더링
              ...
              // 기존 블록 시스템 (plain text, 이전 방식 PDF 텍스트 호환)
      ...
      {showMemoModal && editingHighlight && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}
          ...
          <div className="w-full max-w-sm rounded-2xl p-5 shadow-xl" style={{ backgroundColor: themeStyle.bg }}
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium" style={{ color: themeStyle.text }}>메모</span>
              <div className="flex items-center gap-2">
                <button onClick={() => deleteHighlight(editingHighlight.id)}
                  className="p-1.5 rounded-lg hover:bg-red-500/10" style={{ color: '#ef4444' }}>
                  <Trash2 className="w-4 h-4" />
                </button>
                <button onClick={() => { setShowMemoModal(false); setEditingHighlight(null) }}
                  className="p-1.5 rounded-lg" style={{ color: themeStyle.muted }}>
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            <p className="text-xs mb-3 px-2 py-1.5 rounded-lg" style={{
              backgroundColor: HIGHLIGHT_COLORS[editingHighlight.color],
              color: themeStyle.text,
            }}>"{editingHighlight.selected_text.slice(0, 100)}{editingHighlight.selected_text.length > 100 ? '...' : ''}"</p>
            ...
            <button onClick={saveMemo}
              className="w-full mt-3 py-2 rounded-xl text-sm font-medium text-white"
              style={{ backgroundColor: '#3b82f6' }}>
              저장
            </button>
          </div>
